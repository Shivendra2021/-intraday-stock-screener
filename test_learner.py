...
def test_pattern_keys():
    ...
    check("RSI_50to55 bucket", "RSI_50to55" in key, key)
    check("VOL_veryhigh bucket", "VOL_veryhigh" in key, key)
    check("MACD_pos bucket", "MACD_pos" in key, key)
    check("EMA_full_bull bucket", "EMA_full_bull" in key, key)

    # Case: RSI=35, vol_ratio=0.5, MACD_neg, EMA_bear
    key2 = _build_pattern_key(35.0, 0.5, -0.2, 0.1, "EMA_bear")
    check("RSI_under40 bucket", "RSI_under40" in key2, key2)
    check("VOL_low bucket", "VOL_low" in key2, key2)
    check("MACD_neg bucket (macd < sig)", "MACD_neg" in key2, key2)
    ...
    check("Pattern key is deterministic", k1 == k2, f"k1={k1}, k2={k2}")

    # Boundary: RSI exactly 70 -> RSI_over70
    key3 = _build_pattern_key(70.0, 1.0, 0, 0, "EMA_bear")
    check("RSI=70 -> RSI_over70", "RSI_over70" in key3, key3)
...
def test_smoothing():
    ...
    check("Small sample smoothing (alpha=0.5)", abs(result - expected) < 1e-6,
    ...
    check("Large sample smoothing (alpha=0.3)", abs(result2 - expected2) < 1e-6,
    ...
    check("Zero -> one smoothing", abs(result3 - 0.5) < 1e-6, f"got={result3:.4f}")

    # Both same rate -> stays same
    result4 = _smooth_rate(0.65, 0.65, 20)
    check("Same rate stays same", abs(result4 - 0.65) < 1e-6, f"got={result4:.4f}")
...
def test_anti_pattern():
    ...
    check("Pattern is flagged as anti-pattern (high loser, low winner)",
    ...
    check("Balanced pattern is NOT anti-pattern", not is_anti2,
